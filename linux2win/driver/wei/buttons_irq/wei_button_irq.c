...
//    irq_cnt += 1;
    ...
    g_keyVal = gpio_get_value(S5PV210_GPH2(2));
...
static void x210_key_init(void)
{
    ...
    /* - 1. 申请gpio key */
    ret = gpio_request(S5PV210_GPH2(2), "GPH0");
    if (ret < 0)
    {
...
static int buttons_open(struct inode *tmpNode, struct file *tmpFile)
{
    /* - 打开设备文件的时候，申请中断资源 */
    /* - 对应文件关闭，释放中断资源 */
    request_irq(IRQ_EINT16_31, button_irq_handler, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "left_button", "123");
