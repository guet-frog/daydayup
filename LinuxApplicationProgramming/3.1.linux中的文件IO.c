...
// -- fileReadPoint && fileWritePoint 指针应该是同一个指针，读写都会对filePoint的位置进行改变(write成功后, read成功但是count是0)
// -- 文件重新关闭打开后，filePoint在最初的位置
...
3.1.9.多次打开同一文件
    3.1.9.1、重复打开同一文件读取 // 普通文件可以被重复打开, 设备文件？
        (1) fd1和fd2分别读, 不是接续读写
            说明：使用open两次打开同一个文件时, fd1和fd2所对应的/*文件指针*/是不同的, 2个独立的指针
            -- 是两个独立的filePoint(), 还是两个独立的 vnode 2018-4-12 09:30:28
            -- 应该两个都对, 后者表达更加的准确
            -- fd就是用来索引/*文件管理表*/中的数组成员-vnode
            -- 其中在一个进程中, fd不同则对应的vnode不同, vnode中的filePoint肯定也是不同
            
            文件指针是包含在动态文件的文件管理表中的 // -- 这句话表达应该有问题
...
3.1.10.文件共享的实现方式
    3.1.10.1、什么是文件共享
        (1)文件共享就是/*同一个文件*/(同一个文件指的是同一个inode，同一个pathname(static file))
            ...
            同时(一个打开尚未关闭的同时另一个去操作)操作
        (2)文件共享的意义：通过文件共享来实现多线程同时操作同一个大文件，以减少文件读写时间，提升效率。//也可以来实现进程间的通信, 效率不高

    3.1.10.2、文件共享的3种实现方式
        (1)文件共享的核心：获取多个文件描述符指向同一个文件
...
// 不同进程间文件描述符fd是独立的，查表得到的文件管理表也是独立的，文件管理表是独立的文件指针就是独立的，所以分别读写
