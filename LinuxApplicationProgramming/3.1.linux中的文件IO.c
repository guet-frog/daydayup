...
3.1.2.3、文件操作的一般步骤
	...
	(6)为什么要这么设计？
        ...
        本身对块设备进行操作非常不灵活(最小单位是sector, 常见size=512byte)
		而内存可以按字节为单位来操作，
		而且可以随机操作(内存RAM，random)
...
3.1.2.4、/*重要概念*/ 文件描述符 //文件描述符是针对当前进程而言的
	(1)文件描述符其实实质是一个数字，这个数字在/*一个进程中*/表示一个特定的含义
		...
		以后应用程序如果要操作这一个动态文件，只需要用这个文件描述符进行区分
	(2)一句话讲清楚文件描述符：文件描述符就是用来区分/*一个程序*/打开的多个文件
...
3.1.4.3、exit、_exit、_Exit退出进程
    ...
    (2)我们如何退出程序？//一般情况下，进程 == 程序
        ①: 在main用return，一般原则是程序正常终止return 0
...
3.1.5.open函数的flag详解2
    3.1.5.1、打开不存在的文件时：O_CREAT、O_EXCL
        (5)结论：open中加入O_CREAT后，不管原来这个文件存在与否都能打开成功，
            ...
            //原来的内容会被消除掉（有点类似于先删除原来的文件再创建一个新的）--test 不符合
        (6)这样可能带来一个问题？
            我们本来是想去创建一个新文件的，但是把文件名搞错了弄成了一个老文件名，结果老文件就被意外修改了。
            我们希望的效果是：如果我CREAT要创建的是一个已经存在的名字的文件，则给我报错，不要去创建。
        (7)这个效果就要靠O_EXCL标志和O_CREAT标志来结合使用。
...
3.1.6.文件读写的一些细节
    3.1.6.1、errno和perror
        ...
        (4)linux系统提供了一个函数perror(print error) 读取errno并转化为错误信息字符串, 然后print
...
3.1.6.2、read和write的count/*后面继续讲*/
        ...
        (2)count可以和阻塞非阻塞结合起来
        (3)需要读取或者写入大文件（譬如文件2MB），不会把count设置为2*1024*1024
...
// -- 引言
    ...
    这些对文件的操作最终是如何映射到对硬件的操作，也需要对文件管理策略有所了解
...
3.1.8.lseek详解
    3.1.8.1、lseek函数介绍
        ...
        (3)在动态文件中，我们会通过文件指针来表征这个正在操作的位置。
            ...
            所以文件指针其实是vnode中的一个元素。这个指针表示当前我们正在操作文件流的哪个位置。
...
// -- read write (close open) 可以隐式的改变文件指针
// -- lseek可以显示的改变文件指针
