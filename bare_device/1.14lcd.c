...
1.14.9.1、FIMD结构框图
(1)s5pv210的LCD控制器叫FIMD, FIMD是210内部和图像处理相关的一些部件, 在摄像头等和图像处理有关的部分都可以有关联
(2)FIMD在内部与AHB总线等相连接, 在外部提供RGB接口、I80接口、YUV接口与外部相连接，我们实际使用的是RGB接口

// RGB I80显示器使用, ITU摄像头使用

1.14.9.2、虚拟屏幕叠加（数据手册P1194）
	(1)虚拟屏幕: 屏幕上显示出来的场景是由多个屏幕显示叠加在一起的效果/*新闻联播、各个电视台台标、下方飘动的字幕新闻*//*叠加后的图像发布后为一幅图像*/
	(2)S5PV210的LCD控制器中有5个虚拟屏幕Window0到Window4 
	(3)虚拟屏幕不存在于真实而存在于内存中(显存), 有几个显存区域就有几个虚拟屏幕, /*多个虚拟*/屏幕都被映射到/*一个真实*/的显示屏上面		// 寄存器配置覆盖关系
...
1.14.16.3、bmp图片格式解析及显示
	(1)当前的图像数据是用Image2LCD工具转换得到, 实践中是不可能手工用工具来转的（你想一下Image2LCD也是一个软件，既然它能转，我们也应该可以用软件来转）
