...
3.1.2.3、文件操作的一般步骤
	...
	(6) 为什么要这么设计？
        ...
        本身对块设备进行操作非常不灵活(最小单位是sector, 常见sectorSize = 512 byte)
		内存可以按字节为单位来操作
		可以随机操作(内存RAM, random)

        hello world 首先通过OS的系统调用，加载到OS内核的open write实现
        应用层将数据写入到内核write函数缓冲区就认为写入OK，此时数据还没有写到实际rom中，还在OS内核的缓存buff中
...
3.1.5.open函数的flag详解2
    3.1.5.1、打开不存在的文件时：O_CREAT、O_EXCL
        ...
        (7)flag：O_CREAT + O_EXCL
            没有文件时创建文件，如果文件存在则open返回值fd = -1 // 报错
    ...
    3.1.5.2、O_NONBLOCK
        (4)/*打开一个文件默认就是阻塞式*/, 如果希望以非阻塞的方式打开文件, 则flag中要加O_NONBLOCK标志
        //只用于设备文件, 而不用于普通文件
...
3.1.6.文件读写的一些细节
    ...
    3.1.6.2、read和write的count/*后面继续讲*/
        (1)count和返回值: count参数表示我们想要写或者读的字节数, 返回值表示实际完成的要写或者读的字节数
        (2)count可以和阻塞非阻塞结合起来
...
3.1.8.lseek详解
    3.1.8.1、lseek函数介绍
        ...
        (3)在动态文件中, 通过/*文件指针filePoint*/来表征这个文件/*正在操作*/的位置 -- UI下的光标
            ...
            filePoint其实是vnode中的一个元素, 这个指针表示当前正在操作文件流的哪个位置*/  -- listen video for check 2018-4-16 11:09:23
...
//error     : retrun -1 and [errno is set] to indicate the error
    ...
    3.1.8.3、用lseek构建空洞文件 
        ...
        (4)空洞文件方法对/*多线程共同操作文件*/is helpful
...
// -- 不同进程多次打开相同文件: 2018-4-17 08:53:21
...
/*========================================== lseek test 2018-4-17 08:49:32
fun@open() exe success fd = 3
repeat open file success fd1 = 4
current file point location is 0
fun@write() exe success write_count = 11
current file point location is 11
current file point location is 0
fun@write() exe success write_count = 11
current file point location is 22
current file point location is 11
current file point location is 22
fun@close() exe success
==========================================*/
...
3.1.9.多次打开同一文件
    ...
    3.1.9.4、APPEND的实现原理和其原子操作性说明  //linux文件共享的实现
        (1)O_APPEND为什么能够将分别写改为接续写?  -- filePoint
            ...
            就是移动自己的文件指针的同时也去把别人的文件指针同时移动 --测试这里表述有误2018-4-17 09:01:39
...
3.1.10.文件共享的实现方式
    ...
    3.1.10.2、文件共享的3种实现方式
        ...
        (3)分析文件共享时的核心关注点在于: 分别写/读 还是 接续写/读

    3.1.10.3、再论文件描述符
        (1)文件描述符(nonnegative): 这个数字本质上是/*进程表 中 文件描述符表(数组)*/的index
            进程通过文件描述符作为index去索引查表得到文件表指针, 再间接访问得到这个文件对应的文件表
        (2)文件描述符这个数字是 open系统调用内部由操作系统自动分配
        (3)操作系统规定, fd从0开始依次增加文件描述符表其实就是一个数组(fd是index, 文件表指针是value)
        (4)open时, 内核会从文件描述符表中挑选一个最小的未被使用的数字返回
        (5)fd中0、1、2已经默认被系统占用了, 因此用户进程得到的最小的fd就是3
        (6)运行一个程序得到一个进程时, 内部就默认已经打开了3个文件, 这三个文件对应的fd就是0、1、2
            这三个文件分别叫stdin、stdout、stderr(标准输入、标准输出、标准错误)
        (7)标准输入一般对应的是键盘(可以理解为：0这个fd对应的是键盘的设备文件)
            标准输出一般是LCD显示器(可以理解为：1对应LCD的设备文件)
        (8)printf函数其实就是默认输出到标准输出stdout上
