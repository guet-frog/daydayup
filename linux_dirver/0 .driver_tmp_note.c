...
//5.2.5 33′
 -- cat /proc/sys/kernel/printk			// dmesg
...
//5.2.7 13′
 ...
 -- cat /proc/devices		// 读内存获取chrdev && block dev
 -- 主设备号(major)既是设备编号 也是设备管理表的下标

//5.2.10 1′
/dev/ *		// 每一个文件代表一个设备 或者 访问sys目录下的属性文件
...
22′
device_register()注册设备相关信息到内核。如: 使用哪些gpio 使用哪一个中断号
等其他一些系统资源
...
// 如何判断kernel中是否有该驱动的实现
	(1) 可以通过make menuconfig的查找功能
	(2) 编译过后的源码目录下是否有.o文件
	
// dev_set_drvdata(); dev_get_drvdata();	-- 在show() store()方法中的使用
