...
// ----------------------------------------------
	...
	func(11, 22, *A, **B)	/* - 传参 *A not A -- 效果不一样 */
...
// ----------------------------------------------
...
#if 面向对象
	__xxx__()称之为魔法方法，有特殊功能			// __init__(self, x, y) __str__(self)
	...
	__call__()				// 重写call方法，对象可以直接作为函数指针调用
...
// ----------------------------------------------
	...
	// 框架支持静态、动态文件
	// Django tornado 要求静态文件url -- ./static/index.html -- 区分路由列表
	
	sys.path.insert(1, WSGI_PYTHON_DIR)
	
	if len(sys.argv) < 2:
		sys.exit("python xxx.py module:app")
	...
	数据库可以保证数据的完整性 -- 数据的正确性、有效性

	限定数据类型

	约束
	1，主键约束（1，不能重复、唯一标识，2, 根据主键查找快），主键决定物理存储方式
	2，非空约束
	3，唯一约束（一个表，主键只能有一个，唯一约束可以有多个。物理上存储顺序是由主键维护）
	4，默认约束
	5，外键约束
	...
	where对原始集行进行筛选，满足条件放入结果集
	可以是逐行进行筛选，也可以通过索引跳过多行

	select是取原始集中哪些列放入结果集当中

	多个and时，自左向右进行匹配，当有一个between，匹配最近一个and
#endif
...
// ----------------------------------------------
// 返回函数、匿名函数、装饰器、偏函数 -- note
// ----------------------------------------------

// ----------------------------------------------
// 面向对象
// ----------------------------------------------
	_name -- 外部可以访问，但默认视为私有变量
	...
	静态语言(如：Java)，如果需要传入Animal类型，则传入的对象必须是Animal类型或者其子类
	动态语言(如：python)，只需要传入对象有一个run()方法
	
	动态语言的"鸭子类型"，不要求严格的继承体系
	许多函数接收参数是"file-like object"(文件对象)，不一定要传入真正的文件对象，可以传入一个实现了read()方法的对象
...
// IO编程
// ----------------------------------------------
	open()函数返回的对象包含read()方法 -- 在python中统称为file-like Object
	除了file，也可以是内存的字节流、网络流、自定义流等
	file-like Object不要求从特定类继承，只要包含read()方法
	
	StringIO在内存中创建的file-like Object，常用作临时缓冲
	
	f = open("./gbk.txt", "r", encoding="gbk", errors="ignore")		// 读取非utf-8文本文件，忽略非法编码字符产生的错误
...
// 进程、线程
// ----------------------------------------------
	很多时候，子进程并不是本身，而是一个外部进程，并且要控制子进程的输入输出 -- subprocess启动子进程，控制输入输出
...
// ----------------------------------------------
	...
	二进制文件包含很多无法显示和打印的字符
	文本处理软件(记事本)，处理二进制数据 -- base64二进制到字符串的转换方法
	Base64一种任意二进制到文本字符串的编码方法，常用于在URL、Cookie、网页中传输少量二进制数据
	
	python没有专门处理字节的数据类型，b"str"可以表示字节，字节数据 = 二进制str
	C语言可以通过struct、union来处理字节流
	尽管python不适合编写底层操作字节流的代码，但对性能要求不高的地方，可以用struct解决  -- 解析bmp文件头信息
	struct.pack(">I", 11223344)
	struct.unpack(">IH", b"\xf0\xf0\xf0\xf0\x80\x80")	// >: 大端  I: 四个字节  H:2字节
	
	python提供常见的摘要算法(哈希算法、散列算法) -- 单向函数，不是加密算法(无法通过摘要反推明文)，只能用于放篡改
	通过一个函数，把任意长度的数据转换为一个长度固定的数据串(通常用16进制的字符串表示)
	h = hmac.new(key, message, digestmod="MD5")
	
	
	
	在面向对象(OOP)的设计模式中，decorator被称为装饰模式，通过继承和组合实现
	python除了能支持OOP的decorator，直接从语法层次支持decorator
	python的decorator可以用函数实现，也可以用类实现
	函数调用前打印 'begin call' 和 'end call'
	...
	位图(点阵图)放大模糊，2个像素放大为3个像素，其中一个由另外两个计算得出
	矢量图多用于
	
	二进制文件：01 02 03 31 32 33
	b'\x01'
	b'\x02'
	b'\x03'
	b'1'
	...
	a = '\x31\x31\x32'		// type(a) = str; a = '112'
#endif
