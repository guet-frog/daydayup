...
// ---------------------------------------------
	...
	p.join()	// join()方法可以在当前位置阻塞主进程，带执行join()的进程结束后再继续执行主进程的代码逻辑
	
	// ---------------------------------------------
	// python中使用子进程的常用方法		-- 底层都是调用fork()
	...
	if  __name__ == '__main__':
		print("BBBB")
		p = Process(target = subprocess)	// 创建子进程对象
		print("子进程准备执行")
		p.start()
		p.join()	// 阻塞
		print("子进程执行结束")
	...
	// ---------------------------------------------
	// python用Process创建子类 -- 重写run()方法
	
	// ---------------------------------------------
	// python用进程池实现多任务 -- 主进程结束，进程池释放
	// 这种情况使用子进程，主进程一般用来等待
