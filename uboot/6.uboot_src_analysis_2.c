...
2.6.1.start_armboot		// ./lib_arm/board  -- 整个函数构成了uboot启动的第二阶段
...
2.6.2.start_armboot解析1
	2.6.2.1、init_fnc_t
		...
		(2)init_fnc_t **init_fnc_ptr			// 二重函数指针指向一个函数指针数组		
	...
	2.6.6.2、env_init
		...
		(2)这个函数只是对内存里维护的那一份uboot的env做了基本的初始化或者说是判定是否可用
		   当前还没进行环境变量从SD卡到DDR中的relocate, 因此当前环境变量是不能用的
