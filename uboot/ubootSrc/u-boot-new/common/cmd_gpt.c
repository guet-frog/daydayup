...
static int gpt_default(block_dev_desc_t *blk_dev_desc, const char *str_part)
{
	int ret;
	char *str_disk_guid;
	u8 part_count = 0, pc = 0;
	disk_partition_t *partitions = NULL, *inst;
	...
	printf("EMMC user area partitions entry information:\n");
	pc = part_count;
	for (inst = partitions; pc; inst++, pc--) {
		printf("Partition: %s, sector.%lx, blksz.%luB, blocks.%lu\n", 
				inst->name, inst->start, inst->blksz, inst->size);
	}
