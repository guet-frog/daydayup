...
static void boot_prep_linux(bootm_headers_t *images)
{
	...
	rd_end = images->initrd_end;
	rd_size = images->initrd_end - images->initrd_start;
	debug("initrd_start=0x%lx,initrd_end=0x%lx,initrd_size=0x%lx\n",rd_start,rd_end,rd_size);
	linux_env_init();
	...
	sprintf(env_buf, "0x%lX", rd_end);
	linux_env_set("initrd_end", env_buf);
	...
	sprintf(env_buf, "0x%08X", (uint) (gd->bd->bootargs));
	linux_env_set("boardargs", env_buf);
...
{
        debug("initrd_start=0x%lx,initrd_size=0x%lx\n",images->initrd_start,images->initrd_end);
