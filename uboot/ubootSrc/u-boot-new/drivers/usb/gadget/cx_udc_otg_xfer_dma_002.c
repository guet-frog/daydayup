...
static void dump_ep(void)
{
	int i;
	struct cx_ep *ep = (struct cx_ep *)&memory.ep;

	for (i = 0; i < CX_MAX_ENDPOINTS; i++, ep++)
		debug("ep %p name %s maxpacket %d\n",
			&ep->ep, ep->ep.name, ep->ep.maxpacket);
}

static void dump_regs(void)
{
	debug_cond(DEBUG_INIT, "gotgctl 0x%p\n", &reg->gotgctl);
	debug_cond(DEBUG_INIT, "gotgint 0x%p\n", &reg->gotgint);
	debug_cond(DEBUG_INIT, "gahbcfg 0x%p\n", &reg->gusbcfg);
	debug_cond(DEBUG_INIT, "grstctl 0x%p\n", &reg->grstctl);
	debug_cond(DEBUG_INIT, "gintsts 0x%p\n", &reg->gintsts);
	debug_cond(DEBUG_INIT, "gintmsk 0x%p\n", &reg->gintmsk);
	debug_cond(DEBUG_INIT, "grxstsr 0x%p\n", &reg->grxstsr);
	debug_cond(DEBUG_INIT, "grxstsp 0x%p\n", &reg->grxstsp);
	debug_cond(DEBUG_INIT, "grxfsiz 0x%p\n", &reg->grxfsiz);
	debug_cond(DEBUG_INIT, "gnptxfsiz 0x%p\n", &reg->gnptxfsiz);
	debug_cond(DEBUG_INIT, "dieptxf 0x%p\n", &reg->dieptxf[0]);
	debug_cond(DEBUG_INIT, "dcfg 0x%p\n", &reg->dcfg);
	debug_cond(DEBUG_INIT, "dctl 0x%p\n", &reg->dctl);
	debug_cond(DEBUG_INIT, "dsts 0x%p\n", &reg->dsts);
	debug_cond(DEBUG_INIT, "diepmsk 0x%p\n", &reg->diepmsk);
	debug_cond(DEBUG_INIT, "doepmsk 0x%p\n", &reg->doepmsk);
	debug_cond(DEBUG_INIT, "daint 0x%p\n", &reg->daint);
	debug_cond(DEBUG_INIT, "daintmsk 0x%p\n", &reg->daintmsk);
	debug_cond(DEBUG_INIT, "in_endp 0x%p\n", &reg->in_endp[0]);
	debug_cond(DEBUG_INIT, "out_endp 0x%p\n", &reg->out_endp[0]);
	debug_cond(DEBUG_INIT, "ep 0x%p\n", &reg->ep[0]);
}
