...
static void compl_do_reboot_bootloader(struct usb_ep *ep, 
		struct usb_request *req)
{
	char *argv[] = { "fastboot", NULL};

	do_reset(NULL, 0, 1, argv);
}
...
static void cb_reboot(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (!strcmp(cmd, "reboot-bootloader"))
...
static void cb_getvar(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (!strcmp_l1("version", cmd)) {
		...
		strncat(response, version_string, chars_left);
...
static void cb_oem(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (strncmp("format", cmd + 4, 6) == 0) {
		...
		sprintf(cmdbuf, "gpt write mmc %x \"%s\"", CONFIG_FASTBOOT_FLASH_MMC_DEV, 
			partitions);
...
static void cb_erase(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (!strncmp(cmd, "allerase", 8)) {
		int i;

		dev_desc = get_dev("mmc", CONFIG_FASTBOOT_FLASH_MMC_DEV);
		if (!dev_desc || dev_desc->type == DEV_TYPE_UNKNOWN) {
			error("invalid mmc device");
			fastboot_tx_write_str("invalid mmc device");
			return;
		}

		/* printf partitions infomation */
		print_part_efi(dev_desc);

		/* get partitions name efi */
		get_partitions_name_efi(dev_desc, partitions_name);

		/* erase */
		for (i = 0; partitions_name[i]; i++)
			fb_mmc_erase(partitions_name[i], response);
