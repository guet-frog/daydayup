...
void get_partitions_name_efi(block_dev_desc_t * dev_desc, char **partitions_name)
{
	ALLOC_CACHE_ALIGN_BUFFER_PAD(gpt_header, gpt_head, 1, dev_desc->blksz);
	gpt_entry *gpt_pte = NULL;
	int i = 0;
	...
	for (i = 0; i < le32_to_cpu(gpt_head->num_partition_entries); i++) {       
		...
		partitions_name[i] = strdup(print_efiname(&gpt_pte[i]));                     
	}                                                                          
	...
	const char *name, disk_partition_t *info)
{
	...
	for (i = 1; i < GPT_ENTRY_NUMBERS; i++) {
		...
		if (strncmp((const char *)info->name,name,strlen(info->name)) == 0) {
			/* matched */
			return 0;
		}
	}
		...
		disk_partition_t *partitions, int parts)
{
	...
	for (i = 0; i < parts; i++) {
		...
		/* fill partition offset */
		partitions[i].start = offset;
