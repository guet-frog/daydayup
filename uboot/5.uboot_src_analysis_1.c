...
2.5.2.start.S解析1
    2.5.2.1、不简单的头文件包含
        ...
        (3)#include <version.h>
            // #ifndef __VERSION_H__  -- #define文件作用域
...
2.5.3.start.S解析2
    2.5.3.1、启动代码的16字节头部
        (1)SD卡启动/Nand启动等整个镜像开头需要16字节的校验头 // usb启动不需要, sd卡启动通过mkv210image.c
        (2)uboot这里start.S中在开头位置放了16字节的填充占位, 后面去计算校验和再重新填充
			// #if defined(CONFIG_EVT1)
			// uboot从SD/Nand等启动时的16字节校验头和异常向量表的建立	// 如何建立
...
2.5.4.start.S解析3
    2.5.4.1、设置CPU为SVC模式
        ...
        (2)其实ARM CPU在复位时默认就会进入SVC模式, 但是这里还是使用软件将其置为SVC模式
            整个uboot工作时CPU一直处于SVC模式	// 软件不依赖于硬件的预设
    ...
    2.5.4.3、识别并暂存启动介质选择
        ...
        (2)在210内部有一个寄存器(0xE0000004), 寄存器中的值是硬件根据OM引脚的设置而自动设置值
        (5)moveq r3, #BOOT_MMCSD

    2.5.4.4、设置栈
        (2)在调用函数前初始化栈, 主要原因是在被调用的函数内/*再次执行了调用函数*/
            bl将返回地址存储到lr中, 再次执行函数调用前要先将lr入栈, 否则函数返回时第一层的返回地址就被覆盖了
            // setup stack in sram      // ddr not init
    ...
    2.5.5.1、检查复位状态
        (1)复杂CPU允许多种复位情况: 直接冷上电, 热启动, 睡眠(低功耗)状态下的唤醒等
            //冷上电时DDR是需要初始化, 而热启动或者低功耗状态下的复位则不需要再次初始化DDR
...
2.5.6.start.S解析5
    2.5.6.1、判断当前代码执行位置  // line 110-115
        (1)判定当前代码执行的位置在sram中还是在ddr中     // bl1(uboot的前16K)
...
2.5.9.uboot重定位详解
        (1)D003_7488这个内存地址在sram中, 用来确定通道 	   // 默认通道0启动 -- global variable?
            硬件根据实际电路中sd卡在哪个通道设置变量值     // -- SD0通道(EB000000) SD2通道(EB200000)
        (3)重定位是通过调用movi_bl2_copy函数  // uboot/cpu/s5pc11x/movi.c
		...
		②: MOVI_BL2_BLKCNT
		③: MOVI_PHY_UBOOT_BASE	// physical address

		// test printf param // bl2 position
...
(1)第三次设置栈。这次设置栈还是在DDR中，之前虽然已经在DDR中设置过一次栈了，
	但是本次设置栈的目的是将栈放在比较合适（安全，紧凑而不浪费内存）的地方。
(2)我们实际将栈设置在uboot起始地址上方2MB处，这样安全的栈空间是：2MB-uboot大小-0x1000=1.8MB左右
	这个空间既没有太浪费内存，又足够安全。
...
(1)start_armboot是uboot/lib_arm/board.c中，这是一个C语言实现的函数。这个函数就是uboot的第二阶段。
    这句代码的作用就是将uboot第二阶段执行的函数的地址传给pc，
    实际上就是使用一个远跳转直接跳转到DDR中的第二阶段开始地址处。
(2)远跳转的含义就是这句话/*加载的地址*/和当前运行地址/*current pc*/无关，而和链接地址有关
