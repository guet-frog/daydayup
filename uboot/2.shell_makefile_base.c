...
2.2.1.shell介绍
    2.2.1.1、shell是操作系统的终端命令行
        (1)shell软件系统提供给用户操作的命令行界面, 人机交互的一种方式
        (2)使用shell和操作系统、uboot等软件系统进行交互
            //shell下用sed命令解决\r\n -- sed 命令具有替换功能
    ...
    2.2.1.3、常用shell语言：sh、bash、csh、ksh、perl、python等
        (1)linux下常用的/*脚本语言*/: bash、sh
        (2)perl、python高级shell脚本语言, 常用在网络管理配置等领域(系统运维使用)
        (3)嵌入式常用脚本语言来做配置
        (4)linux下最常用的脚本就是bash

    2.2.1.4、shell脚本的运行机制：解释运行
        (1)脚本程序编写好后源代码即可直接运行/*没有编译链接过程*/
        (2)*/shell程序: 解释运行
            shell程序执行时, shell解析器会逐行的解释shell程序代码, 然后一行一行的去运行  // 顺序结构 
        (3)CPU只认识二进制文件, 不认识源码文件
            脚本程序的编译链接过程不是以脚本程序/*源代码为单位*/
...
2.2.3.shell编程学习1
    2.2.3.2、shell中的变量定义和引用  */变量前 {} not ()
        ...
        //shell弱类型语言, 定义变量时不需要指定数据类型(数字 + 字符串)
...
2.2.4.shell编程学习2
    2.2.4.1、shell中调用linux命令  // shell本质: 将很多Linux命令写成一个文件, 然后执行文件
        (1)直接执行
        (2)反引号括起来执行 // var=`pwd`
...
2.2.7.Makefile基础回顾 
    ...
    2.2.7.2、目标、依赖、命令
        (1)目标就是需要去make xxx的那个xxx, 就是我们最终要生成的东西。
        (2)依赖是用来生成目标的原材料
        (3)命令就是加工方法
    ...
    2.2.7.5、伪目标（.PHONY）
        (1)伪目标是这个目标本身不代表一个文件，执行这个目标不是为了得到某个文件或东西，而是单纯为了执行这个目标下面的命令。
        ...
        //makefile源文件没有可执行权限, 也可以使用make命令
