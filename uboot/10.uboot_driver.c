...
2.10.2.2、mmc_initialize	// inand/sd驱动 -- start_armboot()	-- uboot/drivers/mmc/mmc.c
	...
	(3)sd卡/inand芯片初始化		// 外设复位
		// sd卡内部有主控单元(可自行完成flash读写相关操作), soc初始化sd卡时, 首先发送sd init cmd给sd卡
...
2.10.3.2、s3c_hsmmc_initialize		//uboot/drivers/mmc/s3c_hsmmc.c
	...
	(3)mmc_register功能是进行mmc设备的注册		// struct mmc挂载到mmc_devices  -- list_add_tail() 挂载到系统内核链表中
...
2.10.4.1、struct mmc
	(1)	驱动的设计中有一个关键数据结构，包含一些变量和一些函数指针
		变量用来记录驱动相关的一些属性，函数指针用来记录驱动相关的操作方法。这些变量和函数指针加起来就构成了驱动
		驱动就被抽象为这个结构体
	(2)	一个驱动工作时主要就分几部分:
		驱动构建(构建一个struct mmc然后填充它, 驱动初始化)
